...
function Flow() {
  ...
  tasksPaths = tasks.current.map((currentTasks) => {
    var { paths, longestPath, shortestPath } = tasksPathFinder.findPaths(
      currentTasks,
      finishingTasks[0]
    );
  ...
  tasksLevels.forEach((levelTasks, tasksLevelsIndex) => {
    var lastNode: Node =
      additionalNodes.length > 0
        ? additionalNodes[additionalNodes.length - 1]
    ...
    levelTasks.forEach((levelTask, levelTasksIndex) => {
      ...
      var levelConvergentTasks: string[] = convergentTasks.find((convergents) =>
        convergents.includes(levelTask)
      )!;

      var levelConvergentTaskEdge = levelConvergentTasks
        ? additionalEdges.find((edge) =>
            levelConvergentTasks.includes(edge.label as string)
          )
        : null;
      ...
      if (tasksLevelsIndex > 0) {
        var anteriors =
          tasks.anterior[
            tasks.current.findIndex((currentTask) => currentTask == levelTask)
          ];

        var anteriorTask = anteriors[anteriors.length - 1];

        var anteriorTaskEdge: Edge | any = additionalEdges.find(
          (edge) => edge.label == anteriorTask
        );

        var edge = {
          id: `e${anteriorTaskEdge.target}-${step}`,
          source: anteriorTaskEdge.target,
          target: step,
          type: "straight",
          label: levelTask,
        };
  ...
  for (var node of additionalNodes) {
    if (
      additionalNodes.find(
        (additionalNode) =>
          additionalNode.position.x == node.position.x &&
          additionalNode.position.y == node.position.y &&
          additionalNode.id != node.id
      )
    ) {
  ...
  for (var i = 0; i < tasks.current.length; i++) {
    ...
    tasks.anterior.forEach((anteriorTasks, index) => {
      ...
      if (
        ...
        (anteriorTasks.length > 1 &&
          anteriorTasks[anteriorTasks.length - 1] == currentTask)
      ) {
...
function getFinishingTasks(tasks: Tasks) {
  let finishingTasks: string[] = tasks.current.filter(
    (currentTask) =>
      !tasks.anterior.find((anteriorTasks) =>
        anteriorTasks.includes(currentTask)
      )
  );
...
function getConvergentTasks(tasks: Tasks) {
  ...
  tasks.anterior.forEach((anteriorTasks) => {
    ...
    var todoTasksIndexes = anteriorTasks.map((anteriorTask) =>
      tasks.current.findIndex((currentTask) => currentTask == anteriorTask)
    );
...
function getTasksPath(tasks: Tasks, fromTask: string, toTask: string): any {
  ...
  var fromTaskIndex = tasks.current.findIndex(
    (currentTask) => currentTask == fromTask
  );
  ...
  if (fromTaskSubsequentTasks.includes(toTask)) {
    ...
    return fromTaskSubsequentTasks.map((fromTaskSubsequentTask) => [
      ...path,
      getTasksPath(tasks, fromTaskSubsequentTask, toTask),
    ]);
  }
}
